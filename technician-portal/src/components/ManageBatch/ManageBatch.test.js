...
const testGenerateWorkOrder = async (batchNumber, statusCode, expectedMessage) => {
    ...
    expect(mockAxios.get).toBeCalledWith(`http://${ip}:8000/api/generate-work-order?batch_id=${batchNumber}`, { "responseType": "blob" });
...
const testChangeSensorHeartbeat = async (sensorId, heartbeatValue) => {
    ...
    expect(mockAxios.get).toBeCalledWith(`http://127.0.0.1:8000/api/change-sensor-heartbeat/`, {
...
const testChangeLocation = async (sensorId, locationValue, batchId) => {
    ...
    expect(mockAxios.get).toBeCalledWith(`http://127.0.0.1:8000/api/create-location-log/`, {
